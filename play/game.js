...
const ARENA = 380;          // half-extent of the world, world units
...
// ── Ground ──
// ── Terrain ────────────────────────────────────────────────────────────────
// Gentle drivable hills. groundH() is the single source of truth for elevation:
// the mesh is displaced from it and every ground entity samples it, so nothing
// floats or sinks. Kept low and wide so the truck rides over rather than
// climbing walls, and flat near spawn so you never start inside a slope.
const HILLS = [];
for (let i = 0; i < 14; i++) {
  ...
  HILLS.push({
    ...
    h: 5 + (i % 4) * 3.2,          // 5-14 units tall
    s: 42 + (i % 3) * 26,          // wide and rolling
  });
}
function groundH(x, z) {
  ...
  for (const b of HILLS) {
    ...
    const d2 = (dx * dx + dz * dz) / (b.s * b.s);
    if (d2 < 9) y += b.h * Math.exp(-d2);
  }
  // keep the yard flat so the truck never spawns on a slope
  const dc = Math.sqrt(x * x + z * z);
  if (dc < 55) y *= Math.max(0, (dc - 28) / 27);
  return y;
}
// slope normal, so the truck can pitch and roll with the ground
function groundSlope(x, z, d = 3) {
  return { sx: (groundH(x + d, z) - groundH(x - d, z)) / (2 * d),
           sz: (groundH(x, z + d) - groundH(x, z - d)) / (2 * d) };
}

const groundGeo = new THREE.PlaneGeometry(ARENA * 2, ARENA * 2, 120, 120);
{
  const pos = groundGeo.attributes.position;
  for (let i = 0; i < pos.count; i++) {
    // plane is XY before rotation: y maps to world -z
    pos.setZ(i, groundH(pos.getX(i), -pos.getY(i)));
  }
  pos.needsUpdate = true;
  groundGeo.computeVertexNormals();
}
const ground = new THREE.Mesh(groundGeo, mat(THEME.ground));
...
for (let i = 0; i < 300; i++) {
  ...
  patch.position.set(px, groundH(px, pz) + 0.06 + Math.random() * 0.02, pz);
...
for (let i = 0; i < 150; i++) {                                    // wildflowers
  const a = Math.random() * Math.PI * 2, d = 30 + Math.random() * (ARENA - 40);
  const px = Math.cos(a) * d, pz = Math.sin(a) * d;
...
function building(x, z, w, h, d, color) {
  // Sample the lowest corner and seat the footing there, then extend downward so
  // a building on a slope buries its base instead of hanging off one edge.
  const gy = Math.min(
    groundH(x - w / 2, z - d / 2), groundH(x + w / 2, z - d / 2),
    groundH(x - w / 2, z + d / 2), groundH(x + w / 2, z + d / 2), groundH(x, z)
  );
  h += 4;                                    // extra skirt buried in the slope
  obstacles.push({ x, z, r: Math.max(w, d) * 0.52, hard: true });
  rooftops.push({ x, z, w, d, top: gy + h + 0.8 });
  const b = new THREE.Mesh(box(w, h, d), mat(color));
  b.position.set(x, gy + h / 2 - 2, z);
  b.castShadow = true; b.receiveShadow = true;
  scene.add(b);
  const roof = new THREE.Mesh(box(w + 1.5, 0.8, d + 1.5), mat(0x4a4a55));
  roof.position.set(x, gy + h - 2 + 0.4, z);
...
function tree(x, z, scale = 1) {
  ...
  g.position.set(x, groundH(x, z), z);   // sit on the terrain, not through it
  g.rotation.y = Math.random() * 3;
  scene.add(g);
  obstacles.push({ x, z, r: 1.6 * scale, hard: false });   // trees shudder, don't stop you dead
}
for (let i = 0; i < 95; i++) {           // sparser woodland
  const a = Math.random() * Math.PI * 2;
  const d = 60 + Math.random() * (ARENA - 90);   // spread over the whole arena
...
const SHEEP_TYPES = [
  ...
  { name: 'EVIL CAT', color: 0xe8e4dc, skin: 0xffb3d1, speed: 13.5, hp: 3, scale: 0.9, body: 'cat', voice: 'yowl' },
  { name: 'GIANT',    color: 0xf2ecd8, skin: 0x4a3b2a, speed: 2.2, hp: 46, scale: 2.6, body: 'giant', voice: 'baaTitan' },
];
...
const ZOMBIE_TYPES_Z = [
  ...
  { name: 'EVIL CAT', color: 0x23202a, skin: 0xff3b30, speed: 13.5, hp: 3, scale: 0.9, body: 'cat', voice: 'yowl' },
  { name: 'GIANT',    color: 0x5a6e3a, skin: 0x8fae72, speed: 2.2, hp: 46, scale: 2.6, body: 'giant', voice: 'titan' },
];
...
// Evil cats and giants share the zombie contract ({group, legL, legR, armL, armR})
// so every existing system - damage, grapple, ragdoll, scoring - works on them
// untouched. Only the mesh differs.
function makeEvilCat(t) {
  const g = new THREE.Group();
  const body = new THREE.Mesh(box(1.1, 1.0, 2.6), mat(t.color));
  body.position.y = 1.5; body.castShadow = true; g.add(body);
  const head = new THREE.Mesh(box(1.0, 0.95, 0.95), mat(t.color));
  head.position.set(0, 2.15, 1.4); head.castShadow = true; g.add(head);
  for (const sx of [-1, 1]) {
    const ear = new THREE.Mesh(box(0.3, 0.55, 0.16), mat(t.color));
    ear.position.set(sx * 0.32, 2.75, 1.35); ear.rotation.z = sx * 0.2; g.add(ear);
    const eye = new THREE.Mesh(box(0.26, 0.2, 0.16), mat(t.skin));   // glowing eyes
    eye.position.set(sx * 0.26, 2.25, 1.9); g.add(eye);
  }
  const fang = new THREE.Mesh(box(0.5, 0.22, 0.14), mat(0xffffff));
  fang.position.set(0, 1.92, 1.9); g.add(fang);
  const legL = new THREE.Mesh(box(0.32, 1.4, 0.32), mat(t.color));
  legL.position.set(-0.38, 0.7, 0.8); g.add(legL);
  const legR = legL.clone(); legR.position.x = 0.38; g.add(legR);
  const armL = new THREE.Mesh(box(0.32, 1.4, 0.32), mat(t.color));
  armL.position.set(-0.38, 0.7, -0.8); g.add(armL);
  const armR = armL.clone(); armR.position.x = 0.38; g.add(armR);
  const tail = new THREE.Mesh(box(0.2, 1.6, 0.2), mat(t.color));
  tail.position.set(0, 2.3, -1.5); tail.rotation.x = 0.5; g.add(tail);
  return { group: g, legL, legR, armL, armR };
}

function makeGiant(t) {
  const g = new THREE.Group();
  const body = new THREE.Mesh(box(3.0, 4.4, 1.8), mat(t.color));
  body.position.y = 5.4; body.castShadow = true; g.add(body);
  const head = new THREE.Mesh(box(2.0, 1.9, 1.8), mat(t.skin));
  head.position.y = 8.5; head.castShadow = true; g.add(head);
  const jaw = new THREE.Mesh(box(1.7, 0.5, 1.7), mat(0x2a2018));
  jaw.position.y = 7.75; g.add(jaw);
  for (const sx of [-1, 1]) {
    const eye = new THREE.Mesh(box(0.34, 0.34, 0.2), mat(0xffe14a));
    eye.position.set(sx * 0.5, 8.8, 0.95); g.add(eye);
    const horn = new THREE.Mesh(box(0.5, 1.3, 0.5), mat(0xe8dcc0));
    horn.position.set(sx * 0.95, 9.9, 0); horn.rotation.z = sx * 0.28; g.add(horn);
  }
  const legL = new THREE.Mesh(box(1.1, 3.3, 1.1), mat(t.color));
  legL.position.set(-0.85, 1.65, 0); legL.castShadow = true; g.add(legL);
  const legR = legL.clone(); legR.position.x = 0.85; g.add(legR);
  const armL = new THREE.Mesh(box(0.95, 4.0, 0.95), mat(t.color));
  armL.position.set(-2.0, 5.6, 0); armL.castShadow = true; g.add(armL);
  const armR = armL.clone(); armR.position.x = 2.0; g.add(armR);
  return { group: g, legL, legR, armL, armR };
}

const _baseMake = THEME.edition === 'S' ? makeSheep : makeZombieZ;
function makeZombie(typeIdx) {
  const t = ZOMBIE_TYPES[typeIdx];
  if (t.body === 'cat') return makeEvilCat(t);
  if (t.body === 'giant') return makeGiant(t);
  return _baseMake(typeIdx);
}
...
function spawnZombie() {
  ...
  if (state.wave >= 2) pool.push(6, 6);       // evil cats early - fast flankers
  if (state.wave >= 3) pool.push(7);          // giants soon after, so they are actually met
...
makeAllyTruck(-250, 180, 0x8a5ac9);
makeAllyTruck(260, 240, 0x2f9e9e);
makeAllyTruck(-280, -230, 0xd06a2a);


// ─────────────────────────────────────────────────────────────────────────────
// Allied NINJAS - fast blocky fighters that hunt the horde on foot.
//
// They fill the gap the ally trucks leave: trucks are slow and hold a line,
// ninjas chase. Each picks the nearest live zombie, closes, and slashes on a
// cooldown. Damage routes through hitZombie() like everything else so a boss
// still takes its full HP - a ninja must never be an instant-kill shortcut.
// ─────────────────────────────────────────────────────────────────────────────
const ninjas = [];
function makeNinja(x, z, band) {
  const g = new THREE.Group();
  const suit = 0x1a1d24, skin = 0xe8c9a0;
  const torso = new THREE.Mesh(box(1.5, 2.0, 0.9), mat(suit));
  torso.position.y = 2.6; torso.castShadow = true; g.add(torso);
  const head = new THREE.Mesh(box(1.1, 1.0, 1.0), mat(suit));
  head.position.y = 4.1; head.castShadow = true; g.add(head);
  const eyes = new THREE.Mesh(box(1.12, 0.26, 1.02), mat(skin));
  eyes.position.y = 4.2; g.add(eyes);
  // headband tail, so you can tell them apart at a glance
  const tail = new THREE.Mesh(box(0.22, 0.22, 1.6), mat(band));
  tail.position.set(0, 4.35, 0.9); g.add(tail);
  const bandM = new THREE.Mesh(box(1.18, 0.3, 1.06), mat(band));
  bandM.position.y = 4.5; g.add(bandM);
  const legL = new THREE.Mesh(box(0.5, 1.7, 0.5), mat(suit));
  legL.position.set(-0.42, 0.85, 0); g.add(legL);
  const legR = legL.clone(); legR.position.x = 0.42; g.add(legR);
  const armL = new THREE.Mesh(box(0.42, 1.5, 0.42), mat(suit));
  armL.position.set(-1.05, 2.8, 0); g.add(armL);
  const armR = armL.clone(); armR.position.x = 1.05; g.add(armR);
  // katana
  const blade = new THREE.Mesh(box(0.14, 0.14, 3.0), mat(0xd8e4ef));
  blade.position.set(0, 0, 1.6); armR.add(blade);

  g.position.set(x, 0, z);
  scene.add(g);
  const n = {
    g, kind: 'ninja', legL, legR, legs: [legL, legR], armR, blade, band,
    target: null, cd: 0, slash: 0, walk: Math.random() * 10,
    hp: 60, maxHp: 60, spec: { speed: 15, dmg: 14, cd: 0.55, r: 4.5 },
    dead: false, respawn: 0,
  };
  ninjas.push(n);
  return n;
}
const NINJA_BANDS = [0xd8352a, 0x2f7fb5, 0xf2c200, 0x8a5ac9, 0x3f9e5a, 0xe06aa8];

// ── Beast allies: same hunt/slash brain as the ninja, different bodies and stats.
// A quadruped body shared by the tiger and doberman; the duck gets its own.
function beastBody(g, o) {
  const body = new THREE.Mesh(box(o.bw, o.bh, o.bl), mat(o.main));
  body.position.y = o.y; body.castShadow = true; g.add(body);
  const head = new THREE.Mesh(box(o.bw * 0.8, o.bh * 0.85, o.bh * 0.9), mat(o.main));
  head.position.set(0, o.y + o.bh * 0.55, o.bl * 0.55); head.castShadow = true; g.add(head);
  const snout = new THREE.Mesh(box(o.bw * 0.42, o.bh * 0.4, o.bh * 0.7), mat(o.snout || o.main));
  snout.position.set(0, o.y + o.bh * 0.4, o.bl * 0.55 + o.bh * 0.7); g.add(snout);
  for (const sx of [-1, 1]) {
    const ear = new THREE.Mesh(box(o.bw * 0.22, o.bh * 0.5, 0.2), mat(o.ear || o.main));
    ear.position.set(sx * o.bw * 0.3, o.y + o.bh * 1.05, o.bl * 0.5); g.add(ear);
    const eye = new THREE.Mesh(box(0.18, 0.18, 0.18), mat(0x101010));
    eye.position.set(sx * o.bw * 0.24, o.y + o.bh * 0.62, o.bl * 0.55 + o.bh * 0.66); g.add(eye);
  }
  const legs = [];
  for (const [lx, lz] of [[-1, 1], [1, 1], [-1, -1], [1, -1]]) {
    const L = new THREE.Mesh(box(o.bw * 0.24, o.y, o.bw * 0.24), mat(o.leg || o.main));
    L.position.set(lx * o.bw * 0.34, o.y * 0.5, lz * o.bl * 0.32); L.castShadow = true;
    g.add(L); legs.push(L);
  }
  if (o.stripes) {
    for (let i = 0; i < 4; i++) {
      const st = new THREE.Mesh(box(o.bw * 1.02, o.bh * 0.28, 0.3), mat(0x1a1208));
      st.position.set(0, o.y + o.bh * 0.15, -o.bl * 0.3 + i * (o.bl * 0.22));
      g.add(st);
    }
  }
  const tail = new THREE.Mesh(box(0.28, 0.28, o.bl * 0.6), mat(o.leg || o.main));
  tail.position.set(0, o.y + o.bh * 0.4, -o.bl * 0.7); g.add(tail);
  return { legs, head };
}

function duckBody(g) {
  const body = new THREE.Mesh(box(3.4, 3.0, 4.4), mat(0xffd83a));
  body.position.y = 2.6; body.castShadow = true; g.add(body);
  const head = new THREE.Mesh(box(2.2, 2.2, 2.2), mat(0xffd83a));
  head.position.set(0, 5.4, 1.5); head.castShadow = true; g.add(head);
  const beak = new THREE.Mesh(box(1.3, 0.8, 1.5), mat(0xf07a1e));
  beak.position.set(0, 5.0, 3.0); g.add(beak);
  for (const sx of [-1, 1]) {
    const eye = new THREE.Mesh(box(0.34, 0.34, 0.28), mat(0x101010));
    eye.position.set(sx * 0.62, 5.8, 2.55); g.add(eye);
    const wing = new THREE.Mesh(box(0.5, 1.8, 3.0), mat(0xf5c400));
    wing.position.set(sx * 1.9, 2.9, 0); g.add(wing);
  }
  const legs = [];
  for (const sx of [-1, 1]) {
    const L = new THREE.Mesh(box(0.5, 1.2, 0.5), mat(0xf07a1e));
    L.position.set(sx * 0.8, 0.6, 0); g.add(L); legs.push(L);
  }
  return { legs, head };
}

const HELPERS = [
  { kind: 'ninja',    n: 5, speed: 15, dmg: 14, hp: 60,  cd: 0.55, r: 4.5 },
  { kind: 'tiger',    n: 3, speed: 19, dmg: 22, hp: 90,  cd: 0.7,  r: 5.5 },
  { kind: 'doberman', n: 4, speed: 21, dmg: 11, hp: 55,  cd: 0.4,  r: 4.6 },
  { kind: 'duck',     n: 2, speed: 11, dmg: 30, hp: 140, cd: 1.0,  r: 6.5 },
];

function makeHelper(kind, x, z, i) {
  if (kind === 'ninja') return makeNinja(x, z, NINJA_BANDS[i % NINJA_BANDS.length]);
  const g = new THREE.Group();
  let parts;
  if (kind === 'tiger')    parts = beastBody(g, { bw: 2.6, bh: 2.4, bl: 6.0, y: 2.0, main: 0xf0952a, leg: 0xd97e18, snout: 0xffe0b0, ear: 0x2a1c0c, stripes: true });
  else if (kind === 'doberman') parts = beastBody(g, { bw: 1.9, bh: 2.0, bl: 4.6, y: 1.9, main: 0x241f1c, leg: 0x1a1614, snout: 0x8a5a32, ear: 0x141110 });
  else parts = duckBody(g);
  g.position.set(x, 0, z);
  scene.add(g);
  const spec = HELPERS.find(h => h.kind === kind);
  const n = {
    g, kind, legs: parts.legs, legL: parts.legs[0], legR: parts.legs[1],
    armR: null, target: null, cd: 0, slash: 0, walk: Math.random() * 10,
    hp: spec.hp, maxHp: spec.hp, spec, dead: false, respawn: 0,
  };
  ninjas.push(n);
  return n;
}

// Spread the whole roster around the (now much larger) arena.
let _hi = 0;
for (const h of HELPERS) {
  for (let i = 0; i < h.n; i++) {
    const a = (_hi / 14) * Math.PI * 2, r = 55 + (_hi % 3) * 22;
    makeHelper(h.kind, Math.cos(a) * r, Math.sin(a) * r, i);
    _hi++;
  }
}
...
// ─────────────────────────────────────────────────────────────────────────────
// THE PRINCESS - one giant ally who heals the truck on contact, then rides along.
//
// She is the only full heal in the game, so she is deliberately scarce: one of
// her, she wanders, and after being picked up she rides on the deck and tops the
// truck up slowly. Losing her is not possible - she cannot be damaged - but she
// only fully heals on the pickup itself, so she is a reward for reaching her
// rather than a permanent invulnerability.
// ─────────────────────────────────────────────────────────────────────────────
let princess = null;
function makePrincess() {
  const g = new THREE.Group();
  const GOWN = 0xff5fa8, TRIM = 0xffd6ea, SKIN = 0xf0c9a0, HAIR = 0xffdd55, GOLD = 0xf2c200;
  // giant-scale, same league as the GIANT enemy so she reads as one of them
  const gown = new THREE.Mesh(new THREE.CylinderGeometry(1.4, 3.6, 6.0, 12), mat(GOWN));
  gown.position.y = 3.0; gown.castShadow = true; g.add(gown);
  const sash = new THREE.Mesh(new THREE.CylinderGeometry(1.5, 2.2, 0.7, 12), mat(TRIM));
  sash.position.y = 5.1; g.add(sash);
  const torso = new THREE.Mesh(box(2.2, 2.4, 1.4), mat(GOWN));
  torso.position.y = 6.6; torso.castShadow = true; g.add(torso);
  const head = new THREE.Mesh(box(1.9, 1.9, 1.7), mat(SKIN));
  head.position.y = 8.8; head.castShadow = true; g.add(head);
  const hair = new THREE.Mesh(box(2.15, 2.0, 1.9), mat(HAIR));
  hair.position.set(0, 9.0, -0.15); g.add(hair);
  const braid = new THREE.Mesh(box(0.7, 4.2, 0.7), mat(HAIR));
  braid.position.set(0, 6.6, -1.15); g.add(braid);
  // crown
  const band = new THREE.Mesh(new THREE.CylinderGeometry(1.05, 1.05, 0.45, 10), mat(GOLD));
  band.position.y = 10.0; g.add(band);
  for (let i = 0; i < 5; i++) {
    const a = (i / 5) * Math.PI * 2;
    const spike = new THREE.Mesh(box(0.28, 0.8, 0.28), mat(GOLD));
    spike.position.set(Math.cos(a) * 0.9, 10.6, Math.sin(a) * 0.9); g.add(spike);
  }
  for (const sx of [-1, 1]) {
    const eye = new THREE.Mesh(box(0.24, 0.26, 0.16), mat(0x2a2028));
    eye.position.set(sx * 0.45, 8.95, 0.88); g.add(eye);
    const arm = new THREE.Mesh(box(0.62, 2.6, 0.62), mat(SKIN));
    arm.position.set(sx * 1.5, 6.4, 0); arm.castShadow = true; g.add(arm);
  }
  const mouth = new THREE.Mesh(box(0.5, 0.16, 0.14), mat(0xc0395f));
  mouth.position.set(0, 8.35, 0.9); g.add(mouth);

  // soft glow ring so she is findable across a 380-unit arena
  const halo = new THREE.Mesh(new THREE.RingGeometry(4.2, 5.6, 24),
    new THREE.MeshBasicMaterial({ color: 0xff9ed2, transparent: true, opacity: 0.45, side: THREE.DoubleSide }));
  halo.rotation.x = -Math.PI / 2; halo.position.y = 0.12; g.add(halo);

  // She IS the landmark: 3x scale puts her head above the treeline, so she is
  // spotted from across the arena without needing a marker.
  g.scale.setScalar(3.0);
  const a = Math.random() * Math.PI * 2;
  g.position.set(Math.cos(a) * 130, 0, Math.sin(a) * 130);
  scene.add(g);
  princess = { g, halo, riding: false, walk: 0, wander: 0, dir: new THREE.Vector3(1, 0, 0), healT: 0 };
  return princess;
}
makePrincess();
...
function updatePrincess(dt) {
  ...
  if (pr.riding) {
    // sits on the deck behind the cab, scaled down so she fits the truck
    pr.g.position.copy(truck.position);
    pr.g.rotation.y = truck.rotation.y;
    pr.g.translateZ(1.5); pr.g.translateY(2.6);
    pr.g.scale.setScalar(0.30);
    pr.halo.visible = false;
    // a slow trickle while she rides - never a full heal, that is the pickup
    pr.healT -= dt;
    if (pr.healT <= 0) {
      pr.healT = 3.0;
      if (state.started && !state.over && state.hp < state.maxHp) {
        state.hp = Math.min(state.maxHp, state.hp + 4);
        sfx(880, 0.16, 'sine', 0.07, 1320);
      }
    }
    return;
  }

  // wander, and pulse the halo so she is visible from a distance
  pr.wander -= dt;
  if (pr.wander <= 0) {
    pr.wander = 2 + Math.random() * 3;
    const a = Math.random() * Math.PI * 2;
    pr.dir.set(Math.cos(a), 0, Math.sin(a));
  }
  pr.g.position.addScaledVector(pr.dir, 5 * dt);
  pr.g.position.x = THREE.MathUtils.clamp(pr.g.position.x, -ARENA + 10, ARENA - 10);
  pr.g.position.z = THREE.MathUtils.clamp(pr.g.position.z, -ARENA + 10, ARENA - 10);
  pr.g.rotation.y = Math.atan2(pr.dir.x, pr.dir.z);
  pr.walk += dt * 4;
  pr.g.position.y = groundH(pr.g.position.x, pr.g.position.z)
                  + Math.abs(Math.sin(pr.walk)) * 0.9;   // heavy giant footfalls
  pr.halo.material.opacity = 0.3 + Math.abs(Math.sin(pr.walk * 0.7)) * 0.35;
  pr.halo.rotation.z += dt * 0.6;

  // pickup: touching her heals to full and she climbs aboard
  if (state.started && !state.over && pr.g.position.distanceTo(truck.position) < 22) {
    pr.riding = true;
    state.hp = state.maxHp;
    toast('👑 PRINCESS RESCUED - FULL HEAL');
    for (let i = 0; i < 3; i++) sfx(660 + i * 220, 0.3, 'sine', 0.09, 1600);
    burst(pr.g.position.clone().setY(4), 0xff9ed2, 26, 11);
  }
}
...
function updateNinjas(dt) {
  for (const n of ninjas) {
    if (n.dead) {
      n.respawn -= dt;
      if (n.respawn <= 0) {          // they come back rather than thinning out
        n.dead = false; n.hp = n.maxHp;
        const a = Math.random() * Math.PI * 2, r = 45 + Math.random() * 35;
        n.g.position.set(truck.position.x + Math.cos(a) * r, 0, truck.position.z + Math.sin(a) * r);
        n.g.visible = true;
      }
      continue;
    }
    // retarget when the current one dies or drifts too far
    if (!n.target || n.target.dead || n.target.group.position.distanceTo(n.g.position) > 140) {
      let best = null, bd = 1e9;
      for (const z of zombies) {
        if (z.dead) continue;
        // only chase what threatens the player - the arena is far too big for
        // helpers to wander after a zombie nobody will ever see
        if (z.group.position.distanceTo(truck.position) > 170) continue;
        const d = z.group.position.distanceTo(n.g.position);
        if (d < bd) { bd = d; best = z; }
      }
      n.target = best;
    }

    // No work nearby: escort the player instead of standing in an empty field.
    // Without this the whole roster is invisible in a 380-unit arena.
    if (!n.target) {
      const home = truck.position.clone().sub(n.g.position); home.y = 0;
      const hd = home.length();
      if (hd > 34) {
        home.normalize();
        n.g.rotation.y = Math.atan2(home.x, home.z);
        n.g.position.addScaledVector(home, n.spec.speed * 0.8 * dt);
        n.walk += dt * (n.kind === 'duck' ? 8 : 13);
        const sw = Math.sin(n.walk) * 0.5;
        if (n.legs) n.legs.forEach((L, i) => { L.rotation.x = (i % 2 ? -sw : sw) * (i > 1 ? -1 : 1); });
        n.g.position.y = groundH(n.g.position.x, n.g.position.z)
                       + Math.abs(Math.sin(n.walk * 0.5)) * (n.kind === 'duck' ? 0.5 : 0.25);
        if (n.kind === 'duck') n.g.rotation.z = Math.sin(n.walk * 0.5) * 0.12;
      }
      continue;
    }
    const t = n.target;

    const to = t.group.position.clone().sub(n.g.position); to.y = 0;
    const dist = to.length();
    n.g.rotation.y = Math.atan2(to.x, to.z);

    const sp = n.spec;
    if (dist > sp.r) {
      to.normalize();
      n.g.position.addScaledVector(to, sp.speed * dt);
      n.walk += dt * (n.kind === 'duck' ? 8 : 13);
      const sw = Math.sin(n.walk) * (n.kind === 'ninja' ? 0.7 : 0.55);
      // quadrupeds swing diagonally; two-leggers alternate
      if (n.legs) n.legs.forEach((L, i) => { L.rotation.x = (i % 2 ? -sw : sw) * (i > 1 ? -1 : 1); });
      n.g.position.y = groundH(n.g.position.x, n.g.position.z)
                     + Math.abs(Math.sin(n.walk * 0.5)) * (n.kind === 'duck' ? 0.5 : 0.25);
      if (n.kind === 'duck') n.g.rotation.z = Math.sin(n.walk * 0.5) * 0.12;   // waddle
    } else {
      n.cd -= dt;
      if (n.cd <= 0) {
        n.cd = sp.cd;
        n.slash = 0.18;
        hitZombie(t, sp.dmg, t.group.position.clone().setY(2.2), false);
        // each ally sounds different so you can hear who is working
        if (n.kind === 'ninja') sfx(880, 0.06, 'sawtooth', 0.05, 320);
        else if (n.kind === 'tiger') sfx(150, 0.28, 'sawtooth', 0.16, 60);
        else if (n.kind === 'doberman') sfx(420, 0.1, 'square', 0.1, 180);
        else sfx(600, 0.22, 'square', 0.14, 240);      // QUACK
      }
    }
    n.slash = Math.max(0, n.slash - dt);
    if (n.armR) n.armR.rotation.x = n.slash > 0 ? -2.2 + (0.18 - n.slash) * 12 : -0.3;

    // zombies can take them down; they respawn rather than vanish for good
    for (const z of zombies) {
      if (z.dead) continue;
      if (z.group.position.distanceTo(n.g.position) < 3.0) {
        n.hp -= 9 * dt;
        if (n.hp <= 0) {
          n.dead = true; n.respawn = 8; n.g.visible = false;
          n.g.rotation.z = 0;
          bloodSpray(n.g.position.clone().setY(2), 6, 6);
        }
      }
    }
    n.g.position.x = THREE.MathUtils.clamp(n.g.position.x, -ARENA, ARENA);
    n.g.position.z = THREE.MathUtils.clamp(n.g.position.z, -ARENA, ARENA);
  }
}
...
const VOICES = {
  ...
  // rising, warbling screech - a cat, not a groan
  yowl:   { base: 520, drop: 2.4,  dur: 0.5, warble: 34, cut: 3400, wave: 'sawtooth', vol: 0.13 },
};
...
function tick() {
  ...
  if (!state.over) {
    ...
    // ride the hills: settle onto the surface and lean with the gradient
    {
      const gh = groundH(truck.position.x, truck.position.z);
      truck.position.y += (gh - truck.position.y) * Math.min(1, dt * 9);
      const sl = groundSlope(truck.position.x, truck.position.z);
      const h = truck.rotation.y;
      const pitch = -(Math.sin(h) * sl.sx + Math.cos(h) * sl.sz);
      const roll = (Math.cos(h) * sl.sx - Math.sin(h) * sl.sz);
      truck.rotation.x += (pitch - truck.rotation.x) * Math.min(1, dt * 6);
      truck.rotation.z += (roll - truck.rotation.z) * Math.min(1, dt * 6);
    }
  ...
  for (let i = zombies.length - 1; i >= 0; i--) {
    ...
    z.group.position.y = groundH(z.group.position.x, z.group.position.z)
                       + Math.abs(Math.sin(z.walk * 2)) * 0.16;
